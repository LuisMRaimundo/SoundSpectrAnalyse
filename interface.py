...
from spectral_power import spectral_power, plot_multiple_spectral_powers
...
class SpectrumAnalyzer(QMainWindow):
    ...
    power analysis, dissonance calculation, and visualizations.
    """
    ...
    def analyze_spectral_power(self) -> None:
        ...
        if self.spectral_power_enabled:
            ...
            progress = QProgressDialog("Analyzing spectral power...", "Cancel", 0, len(self.audio_processor.audio_data), self)
            ...
            try:
                error_files = []
                for i, (y, sr, note, file_path) in enumerate(self.audio_processor.audio_data):
                    try:
                        # Update progress
                        progress.setValue(i)
                        progress.setLabelText(f"Analyzing file {i+1}/{len(self.audio_processor.audio_data)}: {note}")
                    
                        # Process events to keep the interface responsive
                        ...
                        save_dir = os.path.join(self.results_directory, f"{note}")
                        os.makedirs(save_dir, exist_ok=True)
                        save_path = os.path.join(save_dir, "spectral_power.png")
                    
                        # Limit signal length to avoid excessive memory consumption
                        max_length = min(len(y), 2**18)  # ~262k samples
                        y_limited = y[:max_length]
                    
                        # Calculate spectral power with limited resources
                        from spectral_power import spectral_power, plot_spectral_power
                        try:
                            # Explicit memory release before starting
                            import gc
                            gc.collect()
                        
                            sp = spectral_power(y_limited, min(2**13, len(y_limited)))
                            plot_spectral_power(sp, label=note, save_path=save_path, show_plot=False)
                            ...
                            error_files.append((note, str(calc_err)))
                            continue
                    
                    except Exception as e:
                        error_files.append((note, str(e)))
                        continue
                    
                    # Memory release after each file
                    import gc
                    gc.collect()
            
                # Close progress dialog
                progress.setValue(len(self.audio_processor.audio_data))
            
                if error_files:
                    error_msg = "Errors in some files:\n\n" + "\n".join([f"{note}: {err}" for note, err in error_files])
